<nav class="btmNav">
    <ul class="display_flex_stretch display_row ulBtmNav">
        <li class="liBtmNav">
            <a href="<?php echo get_permalink(2); ?>" class="display_flex_center btnBtmNav btnBtmNavRecruit01">
                <section class="secBtmNav">
                    <h2 class="t_center h2SecBtmNav">私たちについて</h2>
                    <p class="t_center oswald txtSecBtmNav">COMPANY</p>
                </section>
            </a>
        </li>
        <li class="liBtmNav">
            <a href="<?php echo get_permalink(30); ?>" class="display_flex_center btnBtmNav btnBtmNavCompany01">
                <section class="secBtmNav">
                    <h2 class="t_center h2SecBtmNav">事業内容</h2>
                    <p class="t_center oswald txtSecBtmNav">SERVICE</p>
                </section>
            </a>
        </li>
    </ul>
</nav>